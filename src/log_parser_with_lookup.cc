...
LogParserWithLookup::LogParserWithLookup(DatabaseConnectionPool database, std::string main_table, bool single_lookup_table): LogParser(database)
{
    this->main_table_name = main_table;
    this->single_lookup_table = single_lookup_table;
}

void LogParserWithLookup::create_table()
{
    this->database.create_table(this->main_table_name, this->main_table_schema);
}

void LogParserWithLookup::set_main_table_schema()
{
...
void LogParserWithLookup::set_lookup_table_schema()
{
...
void LogParserWithLookup::insert_log(const std::string &line)
{
    std::vector<std::string> line_field = this->parse_log_line(line);
    line_field = this->update_log_line(line_field);
    this->database.insert_data(this->main_table_name, line_field);
}

void LogParserWithLookup::initialise_lookup()
{
...
void LogParserWithLookup::initialise_schemas(std::string log_type, std::string xml_file_name)
{
    this->load_log_schema(log_type, xml_file_name);
    this->initialise_lookup();
    this->set_main_table_schema();
}

int LogParserWithLookup::get_logcolumn_index_and_col(std::string column_name)
{
...
void LogParserWithLookup::initialise_logcolumn_index_and_logcolumn()
{
    for(LogColumn log_col: this->log_schema)
    {
...
std::map<std::string, int> LogParserWithLookup::initialise_single_lookuptable(std::string log_col_name)
{
    ...
    {
        single_lookup_dict = this->database.fetch_lookup_table_as_dict(log_col_name);
    }
    return single_lookup_dict;
}

void LogParserWithLookup::initialise_lookuptables()
{
...
int LogParserWithLookup::update_string_column(std::string log_one_col_val, std::string lookuptable_name)
{
...
std::string LogParserWithLookup::update_array_column(std::string log_one_col_val, std::string lookuptable_name)
{
...
std::vector<std::string> LogParserWithLookup::update_log_line(std::vector<std::string> log_line)
{
    int i = 0;
...
void LogParserWithLookup::insert_all_and_update_lookup(std::vector<std::string> log_lists)
{
    ...
    for(auto lookuptable: this->lookuptables)
    {
        ...
        {
            this->database.insert_data(lookuptable.first, {column.first, std::to_string(column.second)});
        }
    }
}

void LogParserWithLookup::print_table_schema(std::string table_name)
{
...
void LogParserWithLookup::print_all_table_schema()
{
...
void LogParserWithLookup::print_table_content(std::string table_name, int first_n_line)
{
