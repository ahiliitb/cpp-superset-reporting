...
void BaseLogParser::insert_log_file(const std::string &file_path)
{
    ...
    else
    {
        ...
        size_t simulataneous_logs = 256;
        std::vector<std::string> log_lines;
        // Process the lines with a progress bar
        while (std::getline(file, line))
        {
            // if(log_lines.size() != simulataneous_logs)log_lines.emplace_back(line);

            // Your processing function
            this->insert_log(line);
            // if(log_lines.size() == simulataneous_logs || current_line == total_lines-1)
            // {
